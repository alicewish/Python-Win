...
refer_file_path = file_path_prefix + app_name + "/Base_cn.lproj/Translation Dict.txt"  # 词典文件的地址
output_file_path = file_path_prefix + app_name + "/Base_cn.lproj/" + file_name + ".txt"  # 输出文件的地址
    ...
    for line in fin:
        ...
        if "<string>" in input_line and "<key>" not in input_line and ".title" not in input_line:
            ...
            if len(English) > 1 and re.match(r'\b[A-Z][a-z]{2,}[^.]', English):
                status = True
            if len(English) > 16 and " " not in English:
                status = False
            if status:
